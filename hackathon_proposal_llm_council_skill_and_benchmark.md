# Hackathon Proposal: The LLM Council — A Composable Skill for Rigorous Agent Output Evaluation

## Track: Track 1 (New Agent Skills) + Track 2 (Skill Benchmarking & Evaluation)

## Thesis

Single-pass LLM evaluation of agent outputs is unreliable. Multi-persona evaluation with democratic aggregation catches more issues, produces more calibrated confidence, and is measurable. This should be a reusable, composable skill — not something every team reinvents.

---

## Background & Existing Assets

### LLM Council Methodology (Zhao et al., 2024)
...
### Real Artifacts Available for Benchmarking
...
## Track 1 Contribution: New Agent Skill

### The Council as a Standalone, Domain-Agnostic MCP Tool
...
### Why This Is a Novel Skill

This expands what agents can do in **reasoning and coordination** — it gives any agent the ability to self-evaluate or cross-evaluate with structured rigor. It addresses a gap in the current agent ecosystem: agents can generate outputs, but they lack a composable, principled way to assess whether those outputs are good enough.

---

## Track 2 Contribution: Skill Benchmarking

### Research Question

**Does multi-persona evaluation actually outperform single-pass evaluation?**
...
Comparative data and analysis showing where Council evaluation catches issues that single-pass misses, the "sweet spot" persona count, and guidance for when Council evaluation is worth the additional cost.
...
|---|---|
| Novelty | Fresh project with academic grounding (Zhao et al., 2024) |
| Audience appeal | Broad — everyone building with agents needs to evaluate agent outputs |
| "Process as product" fit | The methodology + comparative data IS the deliverable |
| Real deployment conditions | Benchmarked on production artifacts, not toy tasks |
| Solo-friendly | Analysis-heavy, feasible solo |
| Team-friendly | Natural split: tool building vs. experiment design and analysis |
...
| 4:30–5:30 PM | Write up findings as a structured report. Identify the "sweet spot" persona count. |
...
- **Solo:** Full loop is feasible — implementation is moderate, and the analysis/presentation is where the value lives
- **With a team:** Natural split — someone builds/refines the MCP tool while another designs experiments and analyzes results. A teammate with a different domain could bring their own agent outputs to evaluate, broadening the benchmark
- **Finding a team:** Look for people working on Track 2 who need a concrete evaluation methodology. Offer them the Council framework; they offer additional benchmark scenarios.
